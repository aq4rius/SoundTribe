    ...
    >
      <div className="flex justify-between items-start">
        ...
        {mode === 'full' && onDelete && (
          <div className="space-x-2" onClick={(e) => e.stopPropagation()}>
            ...
            <button onClick={() => onDelete(event._id)} className="btn btn-error btn-sm">
              Delete
            </button>
          </div>
        )}
      ...
      <div className="mt-4 space-y-2">
        <p className="text-base-content">
          {mode === 'compact' ? `${event.description.slice(0, 150)}...` : event.description}
        </p>
        <div className="flex space-x-4 text-sm text-base-content">
          ...
          <span>
            Payment: ${event.paymentAmount} ({event.paymentType})
          </span>
        </div>
        <div className="flex flex-wrap gap-2 mt-2">
          {event.genres.map((genre) => (
            <span
              key={genre._id}
              className="px-2 py-1 bg-base-200 text-base-content rounded-full text-sm"
            >
              {genre.name}
            </span>
          ))}
