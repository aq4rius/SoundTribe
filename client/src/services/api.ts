...
api.interceptors.response.use((response) => {
  ...
  if (error.response && (error.response.status === 401 || error.response.status === 403)) {
    ...
    window.dispatchEvent(new CustomEvent('authError', { detail: error.response.data?.message || 'Authentication error' }));
  }
  // Optionally, dispatch a global error event for other errors
  window.dispatchEvent(new CustomEvent('apiError', { detail: error.response?.data?.message || error.message }));
  return Promise.reject(error);
});
...
export async function getMyProfilesAndEvents() {
  ...
  const profiles = (profilesRes.data || []).map((p: any) => ({ _id: p._id, name: p.stageName, type: 'ArtistProfile' }));
  const events = (eventsRes.data || []).map((e: any) => ({ _id: e._id, name: e.title, type: 'Event' }));
...
export async function getChatTargets(selectedSender: { _id: string, type: string }) {
  ...
  const profiles = ((profilesRes.data?.data) || []).map((p: any) => ({ _id: p._id, name: p.stageName, type: 'ArtistProfile', owner: p.user }));
  const events = ((eventsRes.data?.data) || []).map((e: any) => ({ _id: e._id, name: e.title, type: 'Event', owner: e.owner }));
