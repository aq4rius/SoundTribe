...
import { getUserArtistProfiles, deleteArtistProfile } from '../services/artistProfile';
    ...
    <div className="max-w-7xl mx-auto py-3 px-2 md:px-8">
      ...
      <div className="mb-8">
        <div className="flex justify-between items-center mb-2">
          ...
          <button onClick={() => navigate('/create-event')} className="btn btn-success btn-sm">
            Create New Event
          </button>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {events.map((event) => (
            <EventCard key={event._id} event={event} mode="full" onDelete={handleDeleteEvent} />
          ))}
        </div>
      </div>
