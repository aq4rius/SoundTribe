...
async function fetchGenres() {
  const res = await fetch(`${env.NEXT_PUBLIC_API_URL}/api/genres`);
...
export default function EditEventForm({
    ...
    async function fetchData() {
      ...
      try {
        const [genresRes, eventRes] = await Promise.all([
          fetchGenres(),
          fetch(`${env.NEXT_PUBLIC_API_URL}/api/event-postings/${eventId}`),
        ]);
        setGenres(genresRes);
        ...
        setValue(
          'genres',
          event.genres?.map((g: IGenre | string) => (typeof g === 'string' ? g : g._id)) || [],
        );
        ...
        setError(err instanceof Error ? err.message : 'Failed to load event or genres');
      } finally {
        setLoading(false);
      }
    }
  ...
  const onSubmit = async (data: EventFormValues) => {
    setError(null);
    try {
      const res = await fetch(`${env.NEXT_PUBLIC_API_URL}/api/event-postings/${eventId}`, {
        method: 'PUT',
