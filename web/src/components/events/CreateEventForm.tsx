...
async function createEvent(data: any, token: string | null) {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/event-postings`, {
    method: 'POST',
    headers: {
...
export default function CreateEventForm() {
  ...
  const mutation = useMutation({
    mutationFn: (data: any) => createEvent(data, token),
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        {mutation.isSuccess && (
          <div className="alert alert-success">Event created successfully!</div>
        )}
        <div>
