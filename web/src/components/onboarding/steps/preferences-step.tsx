...
export default function PreferencesStep({ goNext, goBack, onboarding, saveOnboarding }: { goNext: () => void; goBack: () => void; onboarding: OnboardingState | null; saveOnboarding: (data: Partial<OnboardingState>) => void }) {
  const [genres, setGenres] = React.useState<string[]>(onboarding?.preferences?.genres || []);
  const [instruments, setInstruments] = React.useState<string[]>(onboarding?.preferences?.instruments || []);
  const [influences, setInfluences] = React.useState<string[]>(onboarding?.preferences?.influences || []);
  ...
  return (
    <div className="bg-white rounded-xl shadow p-6">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {instrumentOptions.map((i) => (
          <button
            ...
            onClick={() => setInstruments((prev) => prev.includes(i) ? prev.filter(x => x !== i) : [...prev, i])}
          >
            {i}
          </button>
        ))}
      </div>
      <div className="mb-2 text-gray-800 flex items-center gap-2">Influences (optional):
      ...
      <input
        ...
        onChange={e => setInfluences(e.target.value.split(',').map(s => s.trim()).filter(Boolean))}
      />
